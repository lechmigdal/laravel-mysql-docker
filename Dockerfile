...
RUN apt-get update && \
apt-get -y install git zip unzip php5-mcrypt php5-json php5-mysql && \
php5enmod mcrypt && \
php5enmod json && \
a2enmod rewrite && \
...
# Add RDS certificate if you use SSL for AWS RDS
ADD https://s3.amazonaws.com/rds-downloads/rds-combined-ca-bundle.pem /var/www/laravel/rds-combined-ca-bundle.pem
...
COPY gulpfile.js /var/www/laravel/gulpfile.js
